...
function toUrlPath(routePath) {
  const trimmedSrcDir1 = srcDir.replace("./", "").replace("/", "\\");
  const trimmedSrcDir2 = srcDir.replace("./", "").replace("\\", "/");
  const url = routePath.replace(trimmedSrcDir1, "").replace(trimmedSrcDir2, "").replace("+page.svelte", "").replaceAll(" ", "%20").replaceAll("\\", "/").slice(0, -1);
